...
public final class Implementation {
    ...
    public static void setGlobalSelector (ImplementationSelector selector) { Implementation.globalSelector = selector; }

    private static ImplementationSelector getGlobalSelector () {
        if (globalSelector == null) { globalSelector = new DefaultImplementationSelector (); }
    ...
    public static <T extends NamedService> Optional<T> of (Class<T> clazz, ImplementationSelector selector) {
        return of (clazz, selector, null);
    }

    @SuppressWarnings("unchecked")
    public static <T extends NamedService> Optional<T> of (Class<T> clazz, ImplementationSelector selector, ImplementationParams params) {
    ...
    public static <T extends NamedService> Optional<T> ofNew (Class<T> clazz, ImplementationSelector selector) {
        return ofNew (clazz, selector, null);
    }

    public static <T extends NamedService> Optional<T> ofNew (Class<T> clazz, ImplementationSelector selector, ImplementationParams params) {
        ...
        if (LOGGER.isDebugEnabled ()) {
            if (selected.isPresent ()) {
                LOGGER.debug ("Selected implementation is : {}", selected.get ().getServiceImplementationName ());
            } else {
